...
""",connection,transaction);
    ...
    public static async Task<TravelSettingsView> LoadAsync(NpgsqlConnection connection,Guid tenantId,CancellationToken ct=default)
    {
        ...
        await using(var command=new NpgsqlCommand("SELECT inspector_id,inspector_name,inspector_email,source_phone,false,'','',settings_version,updated_at,source_data_json::text FROM public.tenant_inspector_travel_settings WHERE tenant_id=@tenant AND source_active=true ORDER BY inspector_name",connection))
        {command.Parameters.AddWithValue("tenant",tenantId);await using var reader=await command.ExecuteReaderAsync(ct);while(await reader.ReadAsync(ct))inspectors.Add(new(reader.GetGuid(0),reader.GetString(1),reader.GetString(2),reader.GetString(3),reader.GetBoolean(4),reader.GetString(5),reader.GetString(6),reader.GetInt32(7),reader.GetDateTime(8),string.IsNullOrWhiteSpace(reader.GetString(5))?address:reader.GetString(5),JsonSerializer.Deserialize<JsonElement>(reader.GetString(9)))) ;}
